...
use App\Models\ModelSertifikasi;
use App\Models\ModelOrganisasi;
use App\Models\ModelSoft;
use App\Models\ModelHard;
...
class AdminController extends Controller
{
    ...
    public function downloadBerkas($id)
    {
        ...
        // Membuat file zip
        $zipFileName = storage_path('app/' . $dataDokumen->name . '_berkas.zip');
    ...
    public function cv()
    {
        $user = Auth::user();
        $userId = $user->NIK;
        $profil = DB::table('users')->where('NIK', $userId)->first();
        return view ('adminPage.cv.cv', compact('profil'));
    }
    ...
    public function organisasi()
    {
        $user = Auth::user();
        $userId = $user->NIK;
        return view ('adminPage.cv.organisasi');
    }
    ...
    public function sertifikasi()
    {
        $user = Auth::user();
        $userId = $user->NIK;
        $sertifikasi = ModelSertifikasi::where('NIK', $userId)->get();
        return view ('adminPage.cv.sertifikasi', compact('sertifikasi'));
    }
    ...
    public function pendidikan()
    {
        $user = Auth::user();
        $userId = $user->NIK;
        $pendidikan = ModelPendidikan::where('NIK', $userId)->get();
        return view ('adminPage.cv.pendidikan', compact('pendidikan'));
    }
    ...
    public function soft()
    {
        $user = Auth::user();
        $userId = $user->NIK;
        $soft = DB::table('soft_skill')->get();
        return view ('adminPage.cv.soft', compact('soft'));
    }
    ...
    public function hard()
    {
        $user = Auth::user();
        $userId = $user->NIK;
        $hard = ModelHard::where('NIK', $userId)->get();
        return view ('adminPage.cv.hard', compact('hard'));
    }
    ...
    public function addOrganisasi(Request $request)
    {
        $user = Auth::user();
        $userId = $user->NIK;
        $addOrganisasi = new ModelOrganisasi;
        $addOrganisasi->nama_organisasi = $request->input('field_organisasi');
        $addOrganisasi->jabatan = $request->input('field_jabatan');
        $addOrganisasi->masa_kepengurusan = $request->input('field_masa');
        $addOrganisasi->deskripsi = $request->input('field_desc');
        $addOrganisasi->save();
        return redirect('/organisasi')->with('success', 'Data Lowongan Pekerjaan berhasil Ditambahkan.');
    ...
    public function addSertifikasi(Request $request)
    {
        $user = Auth::user();
        $userId = $user->NIK;
        $addSertifikasi = new ModelSertifikasi;
        $addSertifikasi->NIK = $userId;
        $addSertifikasi->lembaga_sertifikasi = $request->input('field_lembaga');
        $addSertifikasi->bidang_sertifikasi = $request->input('field_bidang');
        $addSertifikasi->tanggal_sertifikasi = $request->input('field_tanggal');
        $addSertifikasi->save();
        return redirect('/sertifikasi')->with('success', 'Data Sertifikasi berhasil Ditambahkan.');

    }


    public function addSoftSkill(Request $request)
    {
        $user = Auth::user();
        $userId = $user->NIK;
        $addSoft = new ModelSoft;
        $addSoft->soft_skill = $request->input('field_softSkill');
        $addSoft->save();
        return redirect('/soft')->with('success', 'Data Soft Skill berhasil Ditambahkan.');
    }

    public function addHardSkill(Request $request)
    {
        $user = Auth::user();
        $userId = $user->NIK;
        $addHard = new ModelHard;
        $addHard->NIK = $userId;
        $addHard->hard_skill = $request->input('field_hardSkill');
        $addHard->save();
        return redirect('/soft')->with('success', 'Data Hard Skill berhasil Ditambahkan.');
    }


    public function addDidik(Request $request)
    {
        $user = Auth::user();
        $userId = $user->NIK;
        $addPendidikan = new ModelPendidikan;
        $addPendidikan->NIK = $userId;
        $addPendidikan->instansi = $request->input('field_sekolah');
        $addPendidikan->lokasi = $request->input('field_lokasi');
        $addPendidikan->jurusan = $request->input('field_jurusan');
        $addPendidikan->tahun_masuk = $request->input('field_tahun_masuk');
        $addPendidikan->tahun_lulus = $request->input('field_tahun_lulus');
        $addPendidikan->ipk = $request->input('field_ipk');
        $addPendidikan->save();
