...
class ControllerJob extends Controller
{
    public function index(){
        ...
        $listJob2 = DB::table('job')->take(2)->get();
        return view('landingPage.index', compact('listJob','listJob2'));
    }
    ...
    public function logUser($id)
    {
        $user = Auth::user();
        if ($user->id === $id) {
            $listJob = DB::table('job')->take(4)->get();
    ...
    public function applyJobUser($id, Request $request)
    {
        ...
        if ($user) {
            ...
            $apply->id_company = $request->input('field_company');
            ...
            if ($updateGagal) {
                ...
                return redirect()->route('detailJob', ['id' => $id])->with([
                    'viewData' => $viewData,
                    'error' => 'Apply Job Gagal! Silahkan untuk mengisi kelengkapan Dokumen Anda terlebih dahulu.'
                ]);
                ...
                return redirect()->route('detailJob', ['id' => $id])->with([
                    'viewData' => $viewData,
                    'sukses' => 'Selamat, Apply Job Berhasil! Silahkan Cek progrss aplly job Anda secara berkala.'
                ]);
            } else {
                $viewData = [
                    'userId' => $user->id,
    ...
    public function jobByUser()
    {
        $user = Auth::user();
        $getId = $user->id;
        $job = ModelApply::with(['user', 'job'])->where('id_user', $getId)->get();
        return view('landingPage.jobByUser', compact('user','job'));
    }
